...
`OPENNGS_ORG`/`OPENNGS_NAMESPACE` for the lab. The API authenticates bearer tokens ([#5](https://github.com/OpenNGSOrg/OpenNGS/issues/5)),
...
| **Results back to the order** | The output set joins the order's `Context`. A results service polls `GET /events?after=<last handled>` and resumes across restarts. | Works by polling ([#2](https://github.com/OpenNGSOrg/OpenNGS/issues/2)), or the results service can receive instead: `openngs event relay` pushes to its webhook ([event delivery](event-delivery.md)). |
...
1. ~~**Corrections**~~ ([#1](https://github.com/OpenNGSOrg/OpenNGS/issues/1)) — **done**
...
2. ~~**Who recorded an event**, and authentication~~
   ([#5](https://github.com/OpenNGSOrg/OpenNGS/issues/5)) — **done**. Bearer tokens with
   read/write roles, and `recorded_by` on every event.
3. ~~**Event cursor**~~ ([#2](https://github.com/OpenNGSOrg/OpenNGS/issues/2)) — **done**.
   A consumer follows the log by polling `after` the last event it handled.
4. ~~**Pagination**~~ ([#3](https://github.com/OpenNGSOrg/OpenNGS/issues/3)) — **done**.
...
5. **Adapters**: EHR/FHIR, Illumina run metadata, Nextflow/MultiQC. All written by the
   lab against REST.
6. **Get-or-create** ([#4](https://github.com/OpenNGSOrg/OpenNGS/issues/4)).
...
| **Submission to ENA/SRA** | MIxS metadata from the Specimen facet; `openngs specimen xref add LK-001 biosample:SAMN... ena:ERS...` when the accessions arrive. | Works ([#6](https://github.com/OpenNGSOrg/OpenNGS/issues/6)). The export adapter is still unbuilt. |
| **Asking questions** | "Specimens from site X in summer 2025": `openngs specimen list --subject SITE-X --valid-from 2025-06-01 --valid-to 2025-08-31`. | Works ([#9](https://github.com/OpenNGSOrg/OpenNGS/issues/9)); `valid_time` is on the row, so the parent filter and the date range compose. |
...
1. ~~**Adding an xref to an existing entity**~~
   ([#6](https://github.com/OpenNGSOrg/OpenNGS/issues/6)) — **done**.
2. ~~**Registering controls honestly**~~
   ([#7](https://github.com/OpenNGSOrg/OpenNGS/issues/7)) — **done**.
3. ~~**The `Subject` definition**~~ ([#8](https://github.com/OpenNGSOrg/OpenNGS/issues/8))
   — **done**.
4. ~~**Date-range queries**~~ ([#9](https://github.com/OpenNGSOrg/OpenNGS/issues/9)) —
   **done**. `--valid-from`/`--valid-to` on every list, over all three interfaces.
5. ~~**Bulk loading from a sample sheet**~~
   ([#10](https://github.com/OpenNGSOrg/OpenNGS/issues/10)) — **done**. `openngs ingest
...
own gateway ([#5](https://github.com/OpenNGSOrg/OpenNGS/issues/5)). Alternatively,
...
([#15](https://github.com/OpenNGSOrg/OpenNGS/issues/15)).

### The journey

| Stage | How it maps | Notes |
|---|---|---|
| **Sensing what to do next** | `GET /specimens?without=incoming:derived_from:Extract`, `GET /data-file-sets?without=incoming:used`. One query per sensor, answered and interpreted by the consumer. | Works ([#11](https://github.com/OpenNGSOrg/OpenNGS/issues/11)); composes with the cursor and the date range. |
| **Reacting to change** | Follow the event stream, polling `after` the last event handled, optionally narrowed by `type`. | Polling works ([#2](https://github.com/OpenNGSOrg/OpenNGS/issues/2)); a webhook sink now pushes instead ([event delivery](event-delivery.md)). A broker sink is still unbuilt ([#23](https://github.com/OpenNGSOrg/OpenNGS/issues/23)). |
| **Writing from tasks** | Registering a run is one `POST /batch` of four operations, applied in a single transaction. | Atomic ([#13](https://github.com/OpenNGSOrg/OpenNGS/issues/13)); retries still hit 409s ([#4](https://github.com/OpenNGSOrg/OpenNGS/issues/4)) and parallel workers can still create two nodes for one name ([#14](https://github.com/OpenNGSOrg/OpenNGS/issues/14)). |
| **Launching pipelines on a platform** | `AnalysisRun` at launch with the platform's workflow ID as an xref, `used` the pipeline `Protocol` and the input set; on completion the output set, files, and QC `DataPoint`s. **The run's outcome is a set of `DataPoint`s** on the run, using the `openngs-dp:run_outcome` family. | Fits well ([#17](https://github.com/OpenNGSOrg/OpenNGS/issues/17)); platform IDs arriving late are recorded with `xref add` ([#6](https://github.com/OpenNGSOrg/OpenNGS/issues/6)). |
| **EHR and LIMS** | The DAGs are the adapters; OpenNGS is passive. | Get-or-create ([#4](https://github.com/OpenNGSOrg/OpenNGS/issues/4)) and the manifest loader ([manifests](manifest.md)) cover the mechanics; the adapters are still per-lab code. |
| **Several labs, one orchestrator** | One namespace per API deployment. | Per-request org/namespace ([#16](https://github.com/OpenNGSOrg/OpenNGS/issues/16)). |
| **Operating it** | Long-lived production store. | No migrations for the `Event` table ([#18](https://github.com/OpenNGSOrg/OpenNGS/issues/18)); no metrics beyond `/health`. |
...
|---|---|
| Throughput and backlog per stage | The `without` filter ([#11](https://github.com/OpenNGSOrg/OpenNGS/issues/11)), or the same anti-join as a view ([#19](https://github.com/OpenNGSOrg/OpenNGS/issues/19)) |
| Turnaround time | `valid_time` is on the row now ([#9](https://github.com/OpenNGSOrg/OpenNGS/issues/9)); the views still want an index |
| QC trends by kit lot and operator | A `DataPoint` join walked up to the `Reagent`, on the views from [#19](https://github.com/OpenNGSOrg/OpenNGS/issues/19) |
...
1. ~~**Derived-state queries**~~ ([#11](https://github.com/OpenNGSOrg/OpenNGS/issues/11))
   — **done**.
2. ~~**Outbound event delivery**~~ ([#12](https://github.com/OpenNGSOrg/OpenNGS/issues/12))
   — **done**. `openngs event relay` follows the log and pushes to a webhook; see
   [event delivery](event-delivery.md).
3. ~~**Atomic batch writes**~~ ([#13](https://github.com/OpenNGSOrg/OpenNGS/issues/13))
   — **done**.
4. ~~**Unique index on `name`**~~ ([#14](https://github.com/OpenNGSOrg/OpenNGS/issues/14)) — **done**.
5. **A supported Python client** ([#15](https://github.com/OpenNGSOrg/OpenNGS/issues/15)).
6. **Per-request namespace** ([#16](https://github.com/OpenNGSOrg/OpenNGS/issues/16)).
7. ~~**Run-outcome vocabulary**~~ ([#17](https://github.com/OpenNGSOrg/OpenNGS/issues/17))
   — **done**.
8. **Migrations** ([#18](https://github.com/OpenNGSOrg/OpenNGS/issues/18)).
9. **Reporting views and indexes** ([#19](https://github.com/OpenNGSOrg/OpenNGS/issues/19)).
10. **GraphQL aggregates** ([#21](https://github.com/OpenNGSOrg/OpenNGS/issues/21)).
11. **A read-only database role** ([#22](https://github.com/OpenNGSOrg/OpenNGS/issues/22)).
...
| How are controls (blanks, mock communities) registered? | `--no-parent` on `create`, with the control's role as a facet ([#7](https://github.com/OpenNGSOrg/OpenNGS/issues/7)). A conventional "controls" Subject remains a valid alternative; both are to be documented. |
...
| How is a run's outcome recorded? | As `DataPoint`s on the run (`run_outcome`, `exit_status`, `run_duration_seconds`, ...), never as a status field ([#17](https://github.com/OpenNGSOrg/OpenNGS/issues/17)). |
...
- **Consumers need to follow the log.** Choreography is the architecture. Polling with a
  ...
  rather receive; a broker sink ([#23](https://github.com/OpenNGSOrg/OpenNGS/issues/23))
  is what remains.
